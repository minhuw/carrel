...
The task log provides the concise Mocha failure, stack, and the iteration's exact time range even before platform artifacts are published. Diagnostics written through the smoke runner's `Logger` may exist only in `smoke-test-runner.log`, so do not assume they will be present in the Azure task log.
