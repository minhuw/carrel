...
KNOWN_SUITES="api-folder api-workspace colorize terminal-suggest typescript markdown emmet git git-base configuration-editing github-authentication css html json"
...
API_TESTS_EXTRA_ARGS="--disable-telemetry --disable-experiments --skip-welcome --crash-reporter-directory=$VSCODECRASHDIR --logsPath=$VSCODELOGSDIR --no-cached-data --use-inmemory-secretstorage --disable-extensions --disable-workspace-trust --user-data-dir=$VSCODEUSERDATADIR"
